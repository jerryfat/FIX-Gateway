    ...
    while not done:
        ...
        for i, j in PrevJoystickVals.items():
            if  (round(float(PrevJoystickVals[i][5]), dec) != round(float(joysticks[i][5]), dec)) or \
                ...
                buttons = 1 + 1 << 3 + 1 << 7
                print ("vehicle.mav.manual_control_send: ",vehicle.source_system, int(round((j[5]*1000),0)) ,int(round((j[6]*1000),0)) ,int(round((j[7]*1000),0)) ,int(round((j[9]*1000),0)) ,buttons )
                vehicle.mav.manual_control_send(vehicle.source_system, int(round((j[5]*1000),0)) ,int(round((j[6]*1000),0)) ,int(round((j[7]*1000),0)) ,int(round((j[9]*1000),0)) ,buttons)
