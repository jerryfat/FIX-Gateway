...
GlobalModuleWheels = {} # master dict key:val  package:whl (file name)
allmodules = [] #list of modules to get whl and dependency wheels
if(len(package) >= 1):
    print("using commandline -p package: ",package ," module")
    allmodules.append(package)
    ...
    print("no commandline args, using this py list-modules-imported.py imported modules as the list of modules")
...
def GetPipModuleDeps(module):
    #module = args.module
    print("package: ", module )
    your_package = module
    reqs = [your_package]
    if(len(reqs) >=1) and (len(your_package) >= 1):
        for req in reqs:
            print("package: ", your_package )
...
def GetModuleWhl(allmodules):
    ...
    for i in allmodules: 
        ...
        VerifyAllWheelsList = []
        # skip downloading if 1.)adding module,  if  2.)is ''built-in'' package 3.)exist in global list of allready downloade whhl and package names
        print("@@GlobalModuleWheels.get(i.lower())", GlobalModuleWheels.get(i.lower()) )
        if ( (GlobalModuleWheels.get(i.lower())) == None ) or ( " (built-in)>" not in name ) or  ("ERROR" not in name): 
            component = "[" + i + "]" 
            ...
            if("ERROR" in str(output) ): # no module wheel names to get or get module
                ...
                print("###### ", component, " module ###### \n##### GetModuleWhl() - getting module .whl(s) for pyqtdeploy" )
            ...
            for whl in wheels: # check each line in wheels returned back from pip dowload command in subproccess
                ...
                if ("Successfully downloaded" in whl):                 
                    if("ERROR" not in str(output) ): # if no ERROR in returned stdout from pip download
                        ...
                        print("\n#### add COMPONENT append(s) to sysroot.toml: ####\n")
                        for component in ModuleWheels:
                            ...
                            '''plugin = "\"wheel\""
                            dependencies = str(reqs) # "[" + "\"Python:importlib.resources, Python:os\"" + "]"
                            exclusions = "[" + "\"__main__.py\"" + "]" 
                            whlStr = 'wheel = \"./wheels/' + wheel.replace("  ","") + "\"" 
                            # dict  add to global list of component:whlname
                            print( "before add GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )
                            if (GlobalModuleWheels.get(component.lower()) == None): 
                                GlobalModuleWheels[component.lower()] = './wheels/' + wheel.replace("  ","") 
                               print("GlobalModuleWheels(component.lower()):",component.lower(), GlobalModuleWheels)
                                print( "added GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )
                            print( "afer adding GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )'''
                            # get requirements list from pip
                            #
                            ModuleReqs = []
                            ModuleReqs = GetPipModuleDeps(component)  #allmodules = set() # create  a set of modules
                            if (False): #ModuleReqs: 
                                deps = [] #list
                                for req in ModuleReqs:
                                    print(" dep:", req,"  in ModuleReqs:", ModuleReqs)
                                    if (GlobalModuleWheels.get(req.split(" ")[0]) == None):
                                        deps.append(req.split(" ")[0]) 
                                        print("deps.append reqs deps all", deps)
                                #for getdep in deps: # req.split(" ")[0]) # get wheel for each requirement package in list 
                                     #print("getdep:",getdep)
                                     #GetModuleWhl([getdep])
                                print("deps:",deps)
                                GetModuleWhl(deps)
                            ...
                            print( "before add GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )
                            #
                            if (GlobalModuleWheels.get(component.split(" ")[0]) == None): 
                                GlobalModuleWheels[component.split(" ")[0]] = './wheels/' + wheel.replace("  ","") 
                            print("GlobalModuleWheels(component.lower()):",component.lower(), GlobalModuleWheels)
                            print("added GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )
                            print("afer adding GlobalModuleWheels.get(component.lower())", GlobalModuleWheels.get(component.lower()) )
                                    
                        if (len(ModuleWheels) == len(VerifyAllWheelsList)): 
                            #if error print("VERIFY ERROR , mismatch in Downloaded components vs Saved wheels")
                            #else:
                            #GlobalModuleWheels[component] = whlStr
                            print("VERIFYING SUCCESS len(ModuleWheels):", len(ModuleWheels) ," len(VerifyAllWheelsList)", len(VerifyAllWheelsList), "\n", ModuleWheels,"\n", VerifyAllWheelsList)
                            #exit()
        else:
            print("###### skipped ", i, " ###### --> IGNORING module "+component + " is a (built-in) ...")


print("starting with allmodules:", allmodules)
GetModuleWhl(allmodules)
